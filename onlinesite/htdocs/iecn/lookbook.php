...
<head>
	...
	<?php
        if(isset($_GET["brand"])){
			...
			{
			case "dazzle":
                          $result = mysql_query("SELECT * FROM `dzg_ItemAttributesMap` WHERE dz_parentSequence='7/11/14/' ORDER BY dz_orderWeight ASC");
			  break;  
			case "diamond":
			  $result = mysql_query("SELECT * FROM `dzg_ItemAttributesMap` WHERE dz_parentSequence='7/13/18/' ORDER BY dz_orderWeight ASC");
			  break;  
			case "dzzit":
			  $result = mysql_query("SELECT * FROM `dzg_ItemAttributesMap` WHERE dz_parentSequence='7/12/16/' ORDER BY dz_orderWeight ASC");
			  break;  
			default:
			  $result = mysql_query("SELECT * FROM `dzg_ItemAttributesMap` WHERE dz_parentSequence='7/11/14/' ORDER BY dz_orderWeight ASC");
			}
		...
		<div class="top">
            ...
            <div class="ver">
            	<ul>
                	...
                	<li class="menuitem"><a href="/ieen/lookbook.php?brand=<?php echo $brand;?>">ENGLISH</a></li>
                </ul>
            </div>
        </div>
        <div class="dress">
