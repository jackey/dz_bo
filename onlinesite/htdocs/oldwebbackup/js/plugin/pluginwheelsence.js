			...
			$control.find("a").click(function(){
				...
				if(animationswitch && datavessel){	
					...
					$vessel.animate({marginTop:($vessel.height()*-1*(parseInt(datavessel)+1))+"px"},1000,function(){
						...
						$self.children(".vessel:eq("+(parseInt(datavessel)+1)+")").children("div ,video").animate({opacity: 1},500,function(){
			...
			$infomenu.click(function(){
				...
				if(animationswitch){	
					...
					$vessel.animate({marginTop:(0)+"px"},1000,function(){
							...
							$control.hide();
							animationswitch=true;
						$self.children(".vessel:eq(0)").children("div ,video").animate({opacity: 1},500,function(){
		...
		var Init=function(){
			...
			$self.children(".vessel").each(function(index){ 
				if(index!=config.Page-1){
					$(this).children("div ,video").css({opacity: 0});
				}
			}); 
		};
		...
		var  TableScrollWheel=function(e, delta) {
			...
			if (delta > 0 && ScrollWheelswitch>1 && animationswitch) {
				...
				$vessel.animate({marginTop:(parseInt($vessel.css("margin-top").replace("px",""))+$vessel.height())+"px"},1000,function(){
					...
					$self.children(".vessel:eq("+(ScrollWheelswitch-1)+")").children("div ,video").animate({opacity: 1},500,function(){
						});
					});	
			}
				...
				$vessel.animate({marginTop:(parseInt($vessel.css("margin-top").replace("px",""))-$vessel.height())+"px"},1000,function(){
					...
					$control.find("a").each(function(index){ 
						...
						if(ScrollWheelswitch>=5){
							...
							$self.children(".vessel:eq("+(4)+") , .vessel:eq("+(5)+")").children("div ,video").animate({opacity: 1},500,function(){
		...
		$("#menucorporate").click(function(){
			...
			if(animationswitch){	
					...
					$vessel.animate({marginTop:($vessel.height()*-1*(1))+"px"},1000,function(){
						ScrollWheelswitch=2;						
						animationswitch=true;
						$self.children(".vessel:eq("+(1)+")").children("div ,video").animate({opacity: 1},500,function(){
		...
		$("#menubrand").click(function(){
			...
			if(animationswitch){	
					...
					$vessel.animate({marginTop:($vessel.height()*-1*(2))+"px"},1000,function(){
						ScrollWheelswitch=3;						
						animationswitch=true;
						$self.children(".vessel:eq("+(2)+")").children("div ,video").animate({opacity: 1},500,function(){
		...
		$("#menuinvestors").click(function(){
			...
			if(animationswitch){	
					...
					$vessel.animate({marginTop:($vessel.height()*-1*(3))+"px"},1000,function(){
						ScrollWheelswitch=4;						
						animationswitch=true;
						$self.children(".vessel:eq("+(3)+")").children("div ,video").animate({opacity: 1},500,function(){
		...
		$("#menucareers").click(function(){
			...
			if(animationswitch){	
					...
					$vessel.animate({marginTop:($vessel.height()*-1*(4))+"px"},1000,function(){
						ScrollWheelswitch=5;						
						animationswitch=true;
						$self.children(".vessel:eq("+(4)+")").children("div ,video").animate({opacity: 1},500,function(){
